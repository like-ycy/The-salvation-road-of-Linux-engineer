...
<body>
	<div class="list_con" id="app">
		<h2>To do list</h2>
		<input type="text" v-model="title" id="txt1" class="inputtxt">
		<input type="button" @click="add_task" value="增加" id="btn1" class="inputbtn">
		<ul id="list" class="list">
			<!-- javascript:; # 阻止a标签跳转 -->
			<li v-for="item,key in todolist">
				<span>{{item}}</span>
				<a href="javascript:;" class="up" @click="up_task(key)"> ↑ </a>
				<a href="javascript:;" class="down" @click="down_task(key)"> ↓ </a>
				<a href="javascript:;" class="del" @click="del_task(key)">删除</a>
			</li>
		</ul>
	</div>
	<script>
		const vm = Vue.createApp({
			...
			methods:{
				add_task(){
					...
					// 把新增的计划追加yodolist计划列表中
					this.todolist.unshift(this.title)
				...
				up_task(key){
					...
					let task = this.todolist.splice(key,1)[0]
					console.log(task)
					this.todolist.splice(key-1,0,task)
					// 调用保存数据的方法
					this.save_data()
				},
				down_task(key){
