...
vue-router组件提供的标签跳转不会代码验证和拦截，所以有时希望跳转前进行一些拦截操作，就需要使用js跳转
...
在上面的页面跳转中， 不管 我们使用的router-link组件标签，还是直接通过`this.$router`，实际上都是最终都是\$router来进行页面跳转的。那么除了\$router用于进行页面的跳转以外，在vue-router注册到vue框架以后，vue-router还提供了一个$route对象给我们用于获取不同页面之间的参数传递和接收的操作。
