...
![image-20211112172730403](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112172730403.png)

![image-20211112172752263](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112172752263.png)

![image-20211112172817949](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112172817949.png)

接下来，我们基于oss提供的地址访问上传的静态资源。

![image-20211112173032545](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112173032545.png)

点击详情

![image-20211112173100025](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112173100025.png)

直接访问：http://mofangproject.oss-cn-beijing.aliyuncs.com/pic.png

![image-20211112173125634](8%E4%B8%AA%E4%BA%BA%E4%B8%AD%E5%BF%83.assets/image-20211112173125634.png)
...
AccessKey ID：     LTAI4GAzC4MdRXVQBECjmckc
AccessKey Secret： 0nxNLJ8W7M3ZheIwZhaeImVoJZWG9D
基于上面创建的Bucket存储空间
...
if __name__ == '__main__':
    ...
    AccessKeyID = "LTAI4GAzC4MdRXVQBECjmckc"
    AccessKeySecret = "0nxNLJ8W7M3ZheIwZhaeImVoJZWG9D"
    bucket_name = "mofangproject"
    auth = oss2.Auth(AccessKeyID, AccessKeySecret)
    bucket = oss2.Bucket(auth, endpoint, bucket_name)
    base64_avatar = "base64格式的图片信息"
    b64_image = base64.b64decode(base64_avatar)  # 把base64转换成bytes类型数据
    filename = uuid.uuid4().__str__()
    bucket.put_object(filename, b64_image)
```
...
在`settings.dev`中配置阿里云对象存储的配置信息，代码：
...
def init_app(config_path):
    ...
    # 项目静态文件、上传文件的管理视图
    path = app.BASE_DIR / app.config["STATIC_DIR"]
    admin.add_view(FileAdmin(path, '/static/', name='文件管理', category="项目文件"))

    # 集成redis终端到Admin站点中
    admin.add_view(rediscli.RedisCli(Redis.from_url(app.config.get("REDIS_URL")), name='db_0', endpoint="db_0", category="redis", url="redis/0"))
    admin.add_view(rediscli.RedisCli(Redis.from_url(app.config.get("CHECK_URL")), name='db_1', endpoint="db_1", category="redis", url="redis/1"))
    admin.add_view(rediscli.RedisCli(Redis.from_url(app.config.get("SESSION_URL")), name='db_2', endpoint="db_2", category="redis", url="redis/2"))
...
注意，为了保证图片不被其他第三方恶意爬取，所以我们设置需要验证token才提供给客户端，`application/apps/users/views.py`，代码：

```python
import base64, uuid
from application import oss

@jwt_required() # 验证jwt
def update_avatar(avatar: str) -> Dict[str, Any]:
    """更新用户头像"""
    ...
    # 1. 接受客户端上传的头像信息
    data_list = avatar.split(";base64,")
    data_type = data_list[0].split("/")[-1]  # 文件后缀
    data_b64 = data_list[-1] # base64编码后的文件内容
    b64_image = base64.b64decode(data_b64)             # 把base64转换成bytes类型数据
    filename = uuid.uuid4()                            # 基于UUID生成随机文件名，防止图片名字重复
    # 拼接成完整名称
    file = f"{filename}.{data_type}"
    ret = oss.upload(file, b64_image)
    if ret.status != 200:
        return {
            "errno": status.CODE_UPLOAD_FILE_ERROR,
            "errmsg": message.upload_file_error,
        }

    # 更新数据
    user.avatar = f"{oss.bucket_url}/{file}"
    db.session.commit()
...
```python
CODE_IMAGE_FORMAT_ERROR: int = 1020 # 图片格式有误！
CODE_UPLOAD_FILE_ERROR: int = 1021  # 图片上传失败！
CODE_NO_SUCH_USER: int = 1022       # 用户不存在！
```

错误提示文本,message，代码：

```python
image_format_error: str = "图片格式错误！"
upload_file_error: str = "图片上传失败！"
no_such_user: str = "用户不存在！"
```
...
关闭avatar.html页面，展示底下的setting.html,并更新setting.htm页面的头像。
	...
	apiready = function(){
		...
		new Vue({
			...
			methods:{
                ...
                upload_avatar(ret){
                    // 头像上传处理( vue上传文件 )
                    let access_token = this.game.getdata("access_token") || this.game.getfs("access_token");
                    this.axios.post("",{
                        "jsonrpc": "2.0",
                        "id": this.game.uuid(),
                        "method": "Users.avatar",
                        ...
                        if(response.data.result.errno === 1000 ){
                            ...
                            // 登陆超时
                            this.game.refresh_access_token(this).then(response=>{
                                if(response.data.result.errno === 1000){
                                    this.game.delfs(["access_token"]);
                                    this.game.deldata(["access_token"]);
                                    this.game.setdata({
                                        'access_token': response.data.result.access_token,
                                    });
                                    // 生成access_token以后，再次上传头像
                                    this.upload_avatar(ret);
                                }else{
                                    // refresh_token无法生成新的access_token时，则跳转到登陆页面
                                    this.game.goWin("login", "login.html");
                                }
                            });
                        }else{
