...
![image-20210929180934491](17%E8%AE%A2%E5%8D%95%E7%AE%A1%E7%90%86.assets/image-20210929180934491-16384288958131.png)

上面的效果，可以使用vue-router的嵌套路由来实现，根据不同的子路径，来指定部分内容改变，不会切换公共部分内容。

## 个人中心

订单列表位于用户中心页面，所以我们接下来，先完善头部子组件中的跳转链接。

`src/components/Header.vue`，代码：

```vue
                <span class="el-dropdown-link">
                  <router-link  to="/user"><el-avatar class="avatar" size="50" :src="store.state.user.avatar"></el-avatar></router-link>
                </span>
```

个人中心页面效果展示，个人信息页面：

![image-20210810032254734](17%E8%AE%A2%E5%8D%95%E7%AE%A1%E7%90%86.assets/image-20210810032254734-16384288958142.png)

我的订单页面：

![image-20210810032322132](17%E8%AE%A2%E5%8D%95%E7%AE%A1%E7%90%86.assets/image-20210810032322132-16384288958143.png)
    ...
    def pay_cancel(self, request, pk):
        ...
        with transaction.atomic():
            ...
            try:
                ...
                return Response({"eremsg": "当前订单无法取消，请联系客服工作人员！"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
```
...
const order = reactive({
    order_price: 0,      // 勾选商品的总价格
    discount_price: 0,   // 本次下单的优惠券总价格
    use_coupon: false,   // 用户是否使用优惠
    discount_type: 0,    // 0表示优惠券，1表示积分
    coupon_list:[],      // 用户拥有的可用优惠券列表
    select: -1,          // 当前用户选中的优惠券的下标[-1表示没选中的优惠券]
    fixed: true,         // 底部订单总价是否固定浮动
    pay_type: 0,         // 支付方式[0支付宝，1微信支付，2余额支付]
    ...
    order_list:[],         // 订单列表
    count: 0,              // 订单列表的总数据量
    order_status: -1,      // 当前订单列表对应的订单状态
    order_status_chioces:[], // 状态选项列表
    page: 1,               // 当前订单列表对应的页码
    size: 5,               // 当前订单列表对应的单页数据量
    create_order(pay_type,user_coupon_id,token){
        // 生成订单
    ...
    alipay_page_pay(order_number,token){
        // 获取订单的支付宝支付信息
        return http.get("/payments/alipay/",{
            params: {
                order_number,
            }
        })
    ...
    query_order(order_number, token){
        // 查询订单结果
    ...
    get_order_list(status, token, page, size) {
        // 当前用户的订单列表
        return http.get("/orders/list/", {
            params: {
                status,
...
业务：用户下单在15分钟以后自动判断订单状态如果是0, 则直接改成3,恢复优惠券和用户积分。
```
...
终端下，重启celery。并进入django内置的终端进行异步定时任务的测试。

```bash
# 第一个终端
celery -A fuguangapi worker -l INFO
# 第二个终端
conda activate fuguang
cd ~/Desktop/fuguang/fuguangapi
python manage.py shell
from orders.tasks import order_cancel
ret = order_cancel.apply_async(kwargs={"order_id":3}, countdown=15) # countdown为定时时间，单位：秒
```

效果：

![image-20210811124024579](17%E8%AE%A2%E5%8D%95%E7%AE%A1%E7%90%86.assets/image-20210811124024579-16384288958144.png)

在配置文件`utils/constants.py`中，对定时任务的定时时间设置一个常量
        ...
        with transaction.atomic():
            ...
            try:
                ...
                for course in course_list:
                    ...
                    try:
                        discount_dict = course.discount
                        discount_price = float(discount_dict["price"])
                    except:
...
关于celery在运行过程中， 默认情况下是无法在关机以后重启的。所以我们一般开发中会使用supervisor进程监控来对celery程序进行运行监控！当celery没有启动的情况下，supervisor会自动启动celery，所以我们需要安装supervisor并且编写一个supervisor的控制脚本，在脚本中编写对celery进行启动的命令即可。
...
pip install flower
cd fuguangapi
celery -A fuguangapi flower --port=5555
```
...
```bash
# 在项目根目录下创建存储supervisor配置目录，在fuguangapi创建scripts目录，已经创建则忽略
conda activate fuguang
cd ~/Desktop/fuguangapi
...
创建`fuguang_celery_worker.ini`文件，启动我们项目worker主进程

```bash
cd scripts
touch fuguang_celery_worker.ini
```

```ini
[program:fuguang_celery_worker]
# 启动命令 conda env list
command=/home/moluo/anaconda3/envs/fuguang/bin/celery -A fuguangapi worker -l info -n worker1
# 项目根目录的绝对路径[manage.py所在目录路径]，通过pwd查看
directory=/home/moluo/Desktop/fuguang/fuguangapi
# 项目虚拟环境
enviroment=PATH="/home/moluo/anaconda3/envs/fuguang/bin", C_FORCE_ROOT="yes"
# 运行日志绝对路径
stdout_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.worker.info.log
# 错误日志绝对路径
stderr_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.worker.error.log
# 自动启动，开机自启
autostart=true
# 重启
autorestart=true
...
创建`fuguang_celery_beat.ini`文件，来触发我们的beat定时计划任务

```bash
cd scripts
touch fuguang_celery_beat.ini
```

```ini
[program:fuguang_celery_beat]
# 启动命令 conda env list
command=/home/moluo/anaconda3/envs/fuguang/bin/celery -A fuguangapi  beat -l info
# 项目根目录的绝对路径，通过pwd查看
directory=/home/moluo/Desktop/fuguang/fuguangapi
# 项目虚拟环境
enviroment=PATH="/home/moluo/anaconda3/envs/fuguang/bin"
# 运行日志绝对路径
stdout_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.beat.info.log
# 错误日志绝对路径
stderr_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.beat.error.log
# 自动启动，开机自启
autostart=true
# 重启
autorestart=true
...
创建`fuguang_celery_flower.ini`文件，来启动我们的celery监控管理工具

```bash
cd scripts
touch fuguang_celery_flower.ini
```

```ini
[program:fuguang_celery_flower]
# 启动命令 conda env list
command=/home/moluo/anaconda3/envs/fuguang/bin/celery -A fuguangapi flower --port=5555
# 项目根目录的绝对路径，通过pwd查看
directory=/home/moluo/Desktop/fuguang/fuguangapi
# 项目虚拟环境
enviroment=PATH="/home/moluo/anaconda3/envs/fuguang/bin"
# 输出日志绝对路径
stdout_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.flower.info.log
# 错误日志绝对路径
stderr_logfile=/home/moluo/Desktop/fuguang/fuguangapi/logs/celery.flower.error.log
# 自动启动，开机自启
autostart=true
# 重启
autorestart=true
...
[Service]
Type=forking
ExecStart=/home/moluo/anaconda3/envs/fuguang/bin/supervisord -n -c /home/moluo/Desktop/fuguang/fuguangapi/scripts/supervisord.conf
ExecStop=/home/moluo/anaconda3/envs/fuguang/bin/supervisorctl $OPTIONS shutdown
ExecReload=/home/moluo/anaconda3/envs/fuguang/bin/supervisorctl $OPTIONS reload
KillMode=process
Restart=on-failure
RestartSec=42s
