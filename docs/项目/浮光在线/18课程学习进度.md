    ...
    <div class="all-course-main">
      <div class="allcourse-content">
        <div class="courseitem">
          <div class="img-box">
            <a href=""><img alt="3天javascript入门到崩溃" src="https://fuguang35.oss-cn-beijing.aliyuncs.com/uploads/course/cover/course-9.png" /> </a>
          </div>
          <div class="info-box">
            <div class="title">
              <span>免费课</span>
...
const course = reactive({
    ...
    get_course_direction(){
        return http.get("/courses/diretion")
    },
    // 获取课程分类信息
    get_course_category () {
    ...
    // 获取搜索课程
    search_course() {
        let params = {page: this.page, text: this.text}
        if (this.ordering) {
            params.ordering = this.ordering
        }
...
class UserCourseModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserCourse
        fields = ["course_id","course_cover","course_name","study_time","chapter_id","chapter_orders","chapter_name","lesson_id","lesson_orders","lesson_name","course_type","get_course_type_display","progress", "note", "qa", "code"]
```
...
关于上面学习过程中，用户的笔记、问答和代码，我们也是可以把这些数据进行永久保存在数据库中的。那么，在实现过程中，我们可以这么理解用户与课程与笔记、问答、代码的关系。

![image-20210812094549451](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/image-20210812094549451.png)

```
一个课程：一共4章，20个课时
我们先知道，我们看到了第3章，第4个课时
...
浮光项目中有两种视频：收费视频[需要加密]和免费视频

线上实现视频加密，第三方视频云服务。阿里云、腾讯云、保利威、七牛云、又拍云。。
...
![image-20211203102737446](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/image-20211203102737446.png)

开发文档地址: http://dev.polyv.net/2017/videoproduct/v-playerapi/html5player/html5-docs/

选择云点播，就可以上传视频到保利威。也可以下载保利威的界面软件上传视频，也是可以的。



要开发播放保利威的加密视频开发功能,需要在用户中心->设置->API接口和加密设置.

http://my.polyv.net/secure/setting/api

![1557993716875](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/1557993716875.png)
...
![image-20211203103430069](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/image-20211203103430069.png)

上传视频并加密、接着就要记录视频的VID保存在浮光项目的对应的课时记录中

![1557994524173](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/1557994524173.png)

接下来，我们先解决播放加密视频的功能，然后在考虑把保利威的接口对接到django-simpleUI后台管理站点。

现在我们就手动把刚刚上面上传的视频的VID填写course_lesson里面。

![image-20210812121331997](18%E8%AF%BE%E7%A8%8B%E5%AD%A6%E4%B9%A0%E8%BF%9B%E5%BA%A6.assets/image-20210812121331997.png)
...
参考文档:https://help.polyv.net/index.html#/vod/api/playsafe/token/create_token



根据官方文档的案例，已经有其他人开源针对polvy的token生成的python版本了,我们可以直接拿来使用.
...
从官网上下载保利威web播放器的核心js文件：

文档：https://help.polyv.net/index.html#/vod/js/video_player/function/encryption

播放器js核心文件下载地址：http://player.polyv.net/script/player.js

创建视频播放页面的组件views/Study.vue，代码：
...
course.get_course_info().then(response=>{
  ...
  course.lesson_list = ret
  console.log("50. course.lesson_list", course.lesson_list)
})
...
文档：https://github.com/newpanjing/simpleui_demo
