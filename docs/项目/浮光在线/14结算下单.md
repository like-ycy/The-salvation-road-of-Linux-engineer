...
const cart = reactive({
    checked: false,           // 是否全选
    total_price: 0,           // 勾选商品的总价格
    selected_course_total: 0, // 勾选商品数量
    course_list: [],          // 购物车中的商品列表
    select_course_list:[],    // 购物车中的被勾选的商品列表
...
完成了勾选商品列表展示以后，我们先把优惠券功能和积分功能延后处理，先完成订单的生成。

为了方便以后项目的代码管理和维护，我们再次创建子应用orders来完成接下来的订单功能。
...
```python
订单信息：订单iD，支付状态，支付时间，订单总价格，实付价格，订单标题，订单号，支付方式
订单详情(订单与商品的关系）：商品ID，商品原价、商品实价，优惠方式，订单ID
用户课程（用户与课程的关系）：用户ID，课程ID，学习总时长....
用户学习课程的记录跟踪（用户与课时的关系）：用户ID，课时ID，课程ID，章节ID，学习进度，学习时间....

优惠券：优惠券标题、优惠券面额、优惠券优惠方式、优惠类型、领取方式、起用时间、过期时间....
用户的优惠券（用户与优惠券的关系）: 我们采用redis来记录.
优惠券的使用记录：使用状态、订单ID.....
积分流水：操作方式、积分面值、用户ID、订单ID.....
余额流水：操作方式、货币面值、用户ID、订单ID.....
```
...
上面我们完成了订单信息的添加，但是下单不是一个数据记录而已，而是多张表的同时记录。所以针对这种多个操作连贯进行或者多张表进行连贯操作，为了保证数据的完整性和一致性以及原子性，我们要使用django的ORM提供的事务来完成。
...
事务具有4个特性（ACID），5个隔离等级
  ...
  五个隔离级别（从高到低）： 串行隔离，可重复读，已提交读，未提交读，没有隔离
    原子性（Atomicity）
    一致性（Consistency）
    隔离性（Isolation）[事务隔离级别->幻读，脏读, 不可重复度]
    持久性（Durability）
  ...
  在mysql中有专门的SQl语句来完成事务的操作，事务操作一般有3个步骤：
	设置事务开始  begin;
     ...
     redis.sadd()
     事务的处理[mysql:增删改]
     设置事务的回滚或者提交 rollback / commit;// 这个事务过程中，事务无法对mysql数据库以外的其他操作进行管理
mysql中底层的事务是如何实现事务的回滚操作：do.log逻辑日志
在django等web框架中的ORM框架一般都会实现了事务操作封装，所以我们可以直接使用ORM框架即可完成事务的操作
```
...
class OrderModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ["id", "pay_type", "order_number", "link"]
