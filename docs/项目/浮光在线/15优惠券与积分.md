...
![image-20211129085657537](15%E4%BC%98%E6%83%A0%E5%88%B8%E4%B8%8E%E7%A7%AF%E5%88%86.assets/image-20211129085657537.png)
...
class CouponLog(BaseModel):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="to_coupon", verbose_name="用户", db_constraint=False)
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE, related_name="to_user", verbose_name="优惠券", db_constraint=False)
    order = models.ForeignKey(Order, null=True, blank=True, default=None, on_delete=models.CASCADE, related_name="to_coupon", verbose_name="订单", db_constraint=False)
...
class CouponLogModelAdmin(admin.ModelAdmin):
    ...
    def save_model(self,  request, obj, form, change):
        ...
        if obj.use_status == 0 and obj.use_time == None:
            ...
            pipe.hset(f"{obj.user.id}:{obj.id}","to_diretion", json.dumps(list(obj.coupon.to_diretion.values("diretion__id","diretion__name"))))
...
```sql
-- 优惠券测试数据
INSERT INTO `fg_coupon`
(`id`, `name`, `orders`, `is_show`, `is_delete`, `created_time`, `updated_time`, `discount`, `coupon_type`, `total`, `start_time`, `end_time`, `get_type`, `condition`, `per_limit`, `sale`, `has_total`)
VALUES
(1,'30元通用优惠券',1,1,0,'2021-08-04 10:35:40.569417','2021-08-04 10:35:40.569451',1,0,10000,'2021-08-04 10:35:00.000000','2022-01-02 10:35:00.000000',0,100,1,'-30',10000),
(2,'前端学习通用优惠券',1,1,0,'2021-08-04 10:36:58.401527','2021-08-04 10:36:58.401556',1,1,100,'2021-08-04 10:36:00.000000','2022-08-04 10:36:00.000000',0,0,1,'-50',100),
(3,'Typescript课程专用券',1,1,0,'2021-08-04 10:38:36.134581','2021-08-04 10:38:36.134624',2,3,1000,'2021-08-04 10:38:00.000000','2022-08-04 10:38:00.000000',0,0,1,'*0.88',1000),
(4,'python七夕专用券',1,1,0,'2021-08-04 10:40:08.022904','2021-08-04 10:40:08.022943',1,2,200,'2021-08-04 10:39:00.000000','2021-11-15 10:39:00.000000',1,0,1,'-99',200),
(5,'算法学习优惠券',1,1,0,'2021-08-05 10:05:07.837008','2021-08-05 10:05:07.837041',2,2,1000,'2021-08-05 10:04:00.000000','2021-12-25 10:04:00.000000',0,200,1,'*0.85',1000);

-- 优惠券与学习方向的关系测试数据
INSERT INTO `fg_coupon_course_direction` (`id`, `created_time`, `coupon_id`, `direction_id`) VALUES (1,'2021-08-04 10:36:58.414461',2,1);

-- 优惠券与课程分类的关系测试数据
INSERT INTO `fg_coupon_course_category` (`id`, `created_time`, `category_id`, `coupon_id`) VALUES (1,'2021-08-04 10:40:08.029505',20,4),(2,'2021-08-04 10:40:08.042891',21,4),(3,'2021-08-05 10:05:07.966221',33,5);

-- 优惠券与课程信息的关系测试数据
INSERT INTO `fg_coupon_course` (`id`, `created_time`, `coupon_id`, `course_id`) VALUES (1,'2021-08-04 10:38:36.140929',3,1),(2,'2021-08-04 10:38:36.143166',3,2);

-- 优惠券的发放和使用日志的测试数据
INSERT INTO `fg_coupon_log` (`id`, `name`, `orders`, `is_show`, `is_delete`, `created_time`, `updated_time`, `use_time`, `use_status`, `coupon_id`, `order_id`, `user_id`) VALUES (5,'5',1,1,0,'2021-08-04 12:00:25.051976','2021-08-11 12:59:09.478666',NULL,0,1,45,1),(8,'4',1,1,0,'2021-08-04 12:03:24.331024','2021-08-04 12:30:59.244967',NULL,0,2,NULL,1),(9,'3',1,1,0,'2021-08-04 12:03:31.692397','2021-08-10 11:29:00.120814',NULL,0,3,34,1),(10,'2',1,1,0,'2021-08-04 12:03:38.225438','2021-08-04 12:30:41.830050',NULL,0,4,NULL,1),(11,'1',1,1,0,'2021-08-04 12:09:25.406437','2021-08-04 12:30:35.762828',NULL,0,2,NULL,1),(12,'6',1,1,0,'2021-08-05 10:06:06.036230','2021-08-09 11:44:12.807082',NULL,0,5,1,1);
```

添加测试数据完成以后，因为是通过SQL语句来添加的。务必在Admin站点中对优惠券的发放和使用日志这功能中每一条数据进行一次的更新操作，打开数据详情页不需要修改任何数据，保存即可，这样才能让用户的优惠券信息同步到redis中！！！注意：如果是已经过期的优惠券，则不会被同步到redis中。



### 获取用户本次下单的可用优惠券

服务端获取当前用户拥有的所有优惠券列表，coupon/views.py，代码：
    ...
    def get(self,request):
        ...
        for coupon_key in coupon_id_list:
            ...
            for key,value in coupon_hash.items():
                key = key.decode()
                value = value.decode()
                if key in ["to_course","to_category","to_diretion"]:
                    value = json.loads(value)
                coupon_item[key] = value
            coupon_data.append(coupon_item)

        # todo 获取购物车中的勾选商品[与优惠券的适用范围进行比对，找出能用的优惠券]
...
urlpatterns = [
    path("user/", views.CouponListAPIView.as_view()),
]
```
                 ...
                 <li class="l" :class="{select: order.select === key}" @click="order.select = (order.select === key?-1:key)" v-for="(coupon,key) in order.coupon_list" :key="key">
                  <div class="detail-box more-del-box">
                   <div class="price-box">
                    ...
                    <p class="use-inst l" v-if="coupon.condition>0">满{{coupon.condition}}可用</p>
...
客户端下单以后，本次请求附带使用的 **用户优惠券记录ID**到服务端，服务端进行验证计算，得到正确的实付价格，并从redis中删除用户未使用的优惠券。
...
const get_select_course = () => {
  // 获取购物车中的勾选商品列表
  let token = sessionStorage.token || localStorage.token;
  cart.get_selected(token).then((response) => {
    cart.select_course_list = response.data.cart;
    // 计算本次购物的总价格
    let sum = 0;
    response.data.cart?.forEach((course, key) => {
...
const commit_order = () => {
  ...
  order.create_order(user_coupon_id, token).then((response) => {
    ...
    store.commit(
      "set_cart_total",
      store.state.cart_total - cart.select_course_list.length
    );
  });
};
...
watch(
  ...
  () => {
    ...
    // 针对折扣优惠券，找到最大优惠的课程
    let max_discount = -1;
    for (let course of cart.select_course_list) {
      // 循环本次下单的勾选商品
      // 找到当前优惠券的可用课程
      if (current_coupon.enable_course === "__all__") {
        ...
        if (max_discount !== -1) {
          if (course.price > max_discount.price) {
            // 在每次循环中，那当前循环的课程的价格与之前循环中得到的最大优惠课程的价格进行比较
            max_discount = course;
          }
        } else {
          max_discount = course;
        ...
        current_coupon.enable_course.indexOf(course.id) > -1 &&
        course.price >= parseFloat(current_coupon.condition)
      ) {
        // 只允许没有参与其他优惠券活动的课程使用优惠券，基本所有的平台都不存在折上折的。
        if (course.discount.price === undefined) {
          if (max_discount !== -1) {
    ...
    if (max_discount !== -1) {
      if (current_coupon.discount === "1") {
        // 抵扣优惠券[抵扣的价格就是当前优惠券的价格]
        order.discount_price = parseFloat(Math.abs(current_coupon.sale));
      } else if (current_coupon.discount === "2") {
        // 折扣优惠券]抵扣的价格就是(1-折扣百分比) * 课程原价]
        order.discount_price = parseFloat(
          max_discount.price *
            (1 - parseFloat(current_coupon.sale.replace("*", "")))
        );
      }
    } else {
      order.select = -1;
...
class OrderModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ["id", "pay_type", "order_number", "link", "user_coupon_id"]
...
我们之前在自定义用户模型的时候，已经声明了积分字段，所以此处为了方便后面开发积分功能的时候，能够在admin站点中进行积分的调整使用，所以我们此处在users/admin.ph后台站点配置文件中，配置user用户模型的模型管理器。
...
订单模型orders/models.py，新增积分字段。代码：
...
class CartViewSet(ViewSet):
    ...
    def tegether_select(self, request):
        """全选/全不选"""
        user_id = request.user.id
        selected = int(bool(request.data.get("selected", True)))
        redis = get_redis_connection("cart")
...
客户端分别在购物车和确认下单页面的商品列表中，展示可以使用的积分数量.
...
### 客户端获取当前用户本次下单的可用优惠券列表时，同时获取当前用户拥有的积分。
...
用户最高可使用积分是 用户拥有的所有积分
```

客户端切换不同的优惠类型时，重置积分和优惠券的选择信息，同时当用户选择了积分抵扣时，发送积分数额到服务端。`
...
```vue
              <div class="coupon-content code" v-else>
                <div class="input-box">
                  <el-input-number
                    v-model="order.credit"
                    :step="1"
                    :min="0"
                    :max="order.max_use_credit"
                  ></el-input-number>
                  <a class="convert-btn" @click="conver_credit">兑换</a>
                  <a class="convert-btn" @click="max_conver_credit"
                    >最大积分兑换</a
                  >
                </div>
                <div class="converted-box">
                  <p
                    class="course-title"
                    v-for="course in order.credit_course_list"
                  >
                    课程:<span class="c_name">{{ course.name }}</span>
                    <span class="discount-cash"
                      >{{ course.credit }}积分抵扣:<em>{{
                ...
                <p class="error-msg">
                  本次订单最多可以使用{{
                    order.max_use_credit
                  }}积分，您当前拥有{{ order.has_credit }}积分。(({{
...
class OrderModelSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = ["id", "pay_type", "order_number", "link", "user_coupon_id", "credit"]
        read_only_fields = ["order_number",]
        extra_kwargs = {
            "pay_type": {"write_only": True},
        }
