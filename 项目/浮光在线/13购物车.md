...
```python
    def put(self,request):
        ...
        redis = get_redis_connection("cart")
        cart_hash = redis.hgetall(f"cart_{user_id}")
        course_id_list = [int(key.decode()) for key, _ in cart_hash.items()]
        pipe = redis.pipeline()
        pipe.multi()
  ...
  <div class="cart">
    ...
    <div class="cart-main">
      ...
      <div class="cart-body" id="cartBody">
        ...
        <div class="cart-body-table">
          <div class="item" v-for="(course, key) in cart.course_list" :key="key">
              <div class="item-1">
                  <el-checkbox v-model="course.selected" @change="change_select_course(course)"></el-checkbox>
              </div>
              <div class="item-2">
                  ...
                  <dl class="l has-package">
                    ...
                    <p class="package-item" v-if="course.discount.type">{{course.discount.type}}</p>
                  </dl>
              </div>

              <div class="item-3">
                  <div class="price">
                      <p class="discount-price" v-if="course.discount.price"><em>￥</em><span>{{course.discount.price.toFixed(2)}}</span></p>
                      <p :class="{'original-price': course.discount.price}"><em>￥</em><span>{{course.price.toFixed(2)}}</span></p>
                  </div>
              </div>
              <el-popconfirm
                ...
                iconColor="red"
                title="确定删除吗？"
                @confirm="del_cart(key)"
              >
              <template #reference>
                <div class="item-4"><i class="el-icon-close close"></i></div>
                </template>
              </el-popconfirm>
          </div>
          <div class="cart-body-bot fixed">
...
let token = sessionStorage.token || localStorage.token
cart.get_course(token).then(response=>{
  // 获取购物车中的商品列表
  cart.course_list = response.data.cart;
  // 计算购物车中的所有商品总价格
  get_total()
...
const change_select_course = (course)=>{
  let token = sessionStorage.token || localStorage.token;
  cart.select_course(course.id, course.selected, token).then(response=>{
...
const del_cart = (key)=>{
  // 从购物车中删除指定下标的商品
  console.log( cart.course_list[key] );
}
